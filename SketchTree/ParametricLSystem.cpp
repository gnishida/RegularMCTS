...
void ParametricLSystem::draw(const String& model, RenderManager* renderManager) {
	...
	for (int i = 0; i < model.length(); ++i) {
		...
		if (model[i].name == "[") {
			...
			// 線を描画する
			glutils::drawCylinderY(radius1, radius2, length, glm::vec4(0.9, 0.3, 0.3, 1), modelMat, vertices);
			modelMat = glm::translate(modelMat, glm::vec3(0, length, 0));
			glutils::drawSphere(radius2, glm::vec4(0.9, 0.3, 0.3, 1), modelMat, vertices);
			...
			// 円を描画する
			glutils::drawCircle(radius, length, glm::vec4(0.3, 1, 0.3, 1), mat, vertices);
		}
	}
