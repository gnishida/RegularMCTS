...
GLWidget3D::GLWidget3D(MainWindow* mainWin) : QGLWidget(QGLFormat(QGL::SampleBuffers), (QWidget*)mainWin), lsystem(parametriclsystem::Literal("X", 0, -75, 0, 150, 80)) {
...
void GLWidget3D::resizeSketch(int width, int height) {
	for (int i = 0; i < parametriclsystem::NUM_LAYERS; ++i) {
		...
		painter.drawImage((width - sketch[i].width()) * 0.5, (height - sketch[i].height()) * 0.5, sketch[i]);
		sketch[i] = newImage;
	}
}
...
void GLWidget3D::initializeGL() {
	...
	camera.yrot = 0.0f;
	camera.zrot = 0.0f;
	camera.pos = glm::vec3(0, 80.9, 158);

	//clearImage();

	std::vector<Vertex> vertices;
	glutils::drawAxes(1, 10, glm::mat4(), vertices);
	renderManager.addObject("axis", "", vertices, true);
