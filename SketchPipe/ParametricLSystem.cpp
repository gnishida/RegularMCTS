...
void ParametricLSystem::draw(const String& model, RenderManager* renderManager) {
	...
	for (int i = 0; i < model.length(); ++i) {
		...
		if (model[i].name == "[") {
			...
			// 線を描画する
			glutils::drawCylinderY(1.4, 1.4, length, glm::vec4(0.9, 0.9, 0.9, 1), modelMat, vertices);
			glutils::drawSphere(1.4, glm::vec4(0.9, 0.9, 0.9, 1), glm::rotate(modelMat, deg2rad(-90), glm::vec3(1, 0, 0)), vertices);
	...
	// 背後の壁を描画する
	glutils::drawQuad(300, 300, glm::vec4(1, 1, 1, 1), glm::translate(glm::mat4(), glm::vec3(0, 0, -150)), vertices);
