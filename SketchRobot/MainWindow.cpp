...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
	...
	ui.actionPenColorBody->setCheckable(true);
	ui.actionPenColorBody->setActionGroup(groupPenColor);
	ui.actionPenColorBody->setChecked(true);
	...
	connect(ui.actionPenColorBody, SIGNAL(triggered()), this, SLOT(onPenColorUpdate()));
...
void MainWindow::onRandomGeneration() {
	...
	std::cout << glWidget->model << std::endl;
...
void MainWindow::onPenColorUpdate() {
	if (ui.actionPenColorBody->isChecked()) {
		glWidget->pen.setType(Pen::TYPE_BODY);
	}
}
