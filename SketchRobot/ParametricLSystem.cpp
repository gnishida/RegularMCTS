...
void ParametricLSystem::draw(const String& model, RenderManager* renderManager) {
	...
	for (int i = 0; i < model.length(); ++i) {
		...
		if (model[i].name == "[") {
			...
			glutils::drawEllipsoid(radius, length * 0.5, radius, glm::vec4(0.5, 0.5, 1, 1), glm::translate(modelMat, glm::vec3(0, length * 0.5, 0)), vertices);
			modelMat = glm::translate(modelMat, glm::vec3(0, length, 0));
		} else if (model[i].name == "S") {
			double radius = model[i].param_value1;
			
			// 円を描画する
			glutils::drawSphere(radius, glm::vec4(0.5, 0.5, 1, 1), modelMat, vertices);
		}
	}
